...
public static class SettingsWidgets
{
    ...
    public static void IntSliderWithButtons(Rect rect, ref int value, int minValue, int maxValue, int increment = 1,
        int roundTo = 1)
    {
        if (Widgets.ButtonText(new Rect(rect.xMin, rect.y, rect.height, rect.height), "-", true, false))
        {
        ...
        if (!Widgets.ButtonText(new Rect(rect.xMax - rect.height, rect.y, rect.height, rect.height), "+", true, false))
        {
            return;
        }
    ...
    public static void FloatSliderWithButtons(Rect rect, ref float value, float minValue, float maxValue,
        float increment, bool rounded = false, int intRound = 1, float floatRound = -1f)
    {
        if (Widgets.ButtonText(new Rect(rect.xMin, rect.y, rect.height, rect.height), "-", true, false))
        {
            if (value >= minValue + increment)
