...
public sealed class SrvMod : Mod
{
    ...
    public override void DoSettingsWindowContents(Rect rect)
    {
        ...
        list.Gap(6);
        {
            var fullRect = list.GetRect(Text.LineHeight);
            var leftRect = fullRect.LeftHalf().RightPartPixels(325).Rounded();
            var rightRect = fullRect.RightHalf().Rounded();

            Widgets.Label(leftRect,
                "SRV_Label_VinegarBarrel_MaxCapacity".Translate(SrvSettings.VinegarBarrel_MaxCapacity));
        ...
        list.Gap(6);
        {
            ...
            Widgets.Label(leftRect,
                "SRV_Label_VinegarBarrel_FermentDays".Translate(SrvSettings.VinegarBarrel_FermentDays));
            if (Mouse.IsOver(leftRect))
            {
                Widgets.DrawHighlight(leftRect);
            }
        ...
        list.Gap();
        {
            ...
            Widgets.Label(leftRect,
                "SRV_Label_CheeseBarrel_MaxCapacity".Translate(SrvSettings.CheeseBarrel_MaxCapacity));
            if (Mouse.IsOver(leftRect))
            {
                Widgets.DrawHighlight(leftRect);
            }
        ...
        list.Gap(6);
        {
            ...
            Widgets.Label(leftRect,
                "SRV_Label_CharcoalPit_CharcoalPerWoodLog".Translate(SrvSettings.CharcoalPit_CharcoalPerWoodLog));
            if (Mouse.IsOver(leftRect))
            {
                Widgets.DrawHighlight(leftRect);
            }
            ...
            SettingsWidgets.FloatSliderWithButtons(rightRect, ref SrvSettings.CharcoalPit_CharcoalPerWoodLog, 0.5f, 5f,
                0.5f, floatRound: 0.5f);
        }
        list.Gap();
        {
            var fullRect = list.GetRect(Text.LineHeight);
        ...
        list.Gap(6);
        {
            var fullRect = list.GetRect(Text.LineHeight);
            var leftRect = fullRect.LeftHalf().RightPartPixels(325).Rounded();
            var rightRect = fullRect.RightHalf().RightPartPixels(325).Rounded();

            Widgets.CheckboxLabeled(leftRect, Static.Label_AllowPositiveLetter,
                ref SrvSettings.Snare_AllowPositiveNotification);
            if (Mouse.IsOver(leftRect))
            {
                Widgets.DrawHighlight(leftRect);
            }
            ...
            Widgets.CheckboxLabeled(rightRect, Static.Label_AllowNegativeLetter,
                ref SrvSettings.Snare_AllowNegativeNotification);
        ...
        list.Gap(6);
        {
            ...
            if (Widgets.ButtonText(rightRect, SrvSettings.Snare_NotificationType.ToHumanString()))
            {
                ...
                {
                    fmo.Add(new FloatMenuOption(nt.ToHumanString(),
                        delegate { SrvSettings.Snare_NotificationType = nt; }));
                }
        ...
        list.Gap();
        {
            ...
            SettingsWidgets.FloatSliderWithButtons(rightRect, ref SrvSettings.GenStep_PlantDensity, 0f, 5f, 0.5f,
                floatRound: 0.5f);
        }
        list.End();
    }
