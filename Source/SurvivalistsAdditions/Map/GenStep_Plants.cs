...
public class GenStep_Plants : GenStep
{
    ...
    public override void Generate(Map map, GenStepParams parms)
    {
        ...
        Predicate<IntVec3> validWater = c => c.GetEdifice(map) == null && !c.Roofed(map) &&
                                             map.thingGrid.ThingsListAtFast(c).NullOrEmpty() &&
                                             (c.GetTerrain(map) == TerrainDefOf.WaterShallow ||
                                              c.GetTerrain(map) == TerrainDefOf.WaterMovingShallow ||
                                              c.GetTerrain(map) == SrvDefOf.Marsh);

        // Validate water and shore cells
        Predicate<IntVec3> waterAndShoreValidator = c =>
        ...
        Predicate<IntVec3> shoreValidator = c =>
            c.GetEdifice(map) == null && !c.Roofed(map) && map.thingGrid.ThingsListAtFast(c).NullOrEmpty() &&
            (c.GetTerrain(map).fertility > 0 || c.GetTerrain(map) == SrvDefOf.Mud);
        ...
        for (int ht = 0, hs = 0; ht < timesToSpawn; ht++)
        {
            ...
            CellFinderLoose.TryFindRandomNotEdgeCellWith(10, validWater, map, out var intVec);
